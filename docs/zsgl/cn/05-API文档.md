...
| 1.26.920.1 | 2026-09-20 | gt | 与代码一致性修订（T-903）：B3 补充说明——LogRecorder 日志键不属于跨页信号契约（见 04-数据库设计 §3.1）；A9/A10/B 表内容经核对与代码一致，未改动 |
...
> 注（2026-09-20）：崩溃日志系统的 localStorage 键（`zsgl_log_<world>_<tabId>` 缓冲、`_hb_<tabId>` 心跳、`_last` 转存）**不属于**跨页任务信号契约，为 LogRecorder 内部存储，键清单见《04-数据库设计》§3.1。
