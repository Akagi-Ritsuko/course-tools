...
| 2026-09-21 | T-004 实机复现与崩溃日志回收：单标签 3 次复现（刷新即崩，~70s）；经 Edge LevelDB 旁路提取崩溃前日志（自动下载被"多个自动下载"限制拦截）；结论=扩展 JS 静默/堆稳定，嫌疑聚焦 GPU/DRM 解码与 readyState=0 抢跑点击；另发现恢复导出"下载失败仍删键"致日志丢失缺陷与站点 debugger-checker 反调试（开 DevTools 会杀页面）。提取件本地保存于 .trae/log-extract/（不入库） | .trae/log-extract/、docs/tasks.md、.trae/documents/zsgl-debug-handoff.md | T-004、T-002 |
