...
| T-004 | P1 核心排查：视频起播时 CPU 100% 整浏览器冻死。**诱因已锁定（2026-09-21 缓解实验阶梯）**：实验 B（无扩展+播放 4min）干净 → E0（扩展注入+auto=false 不起播 3.5min）干净 → **M1（auto=true，工具只模拟点击任务卡、对播放器零干预）风暴复现**（N008086，renderer 93→132% 持续 >5.5min 未缓解，主进程/GPU 正常）——诱因=**taskDiv.click() 模拟点击触发站点播放器/DRM 容器重建**，与播放器干预动作无关。M2 待做：去掉模拟点击改纯旁观（ended 监听推进）验证。证据 .trae/log-extract/{cpu-sample-E0,M1*} | doing | P0 | handoff §四P1 | 03-详细设计 §5/§10 |
