...
export class ZsglVideo extends ZsglTask {
  ...
  public Stop(): Promise<void> {
    ...
    this.stopKeepAlive();
    this.runCleanup();
    // 移除完成标志 hook,释放对本实例的强引用
    removeHttpRequestHook(
      ZSGL_CONSTANTS.HTTP_ENDPOINTS.SET_CONTENT_FINISHED,
      this,
    );
  ...
  private waitForVideoSourceAndPlay(): void {
    ...
    const tryPlay = () => {
      ...
      if (attemptCount >= 8 && (this.video.src || this.video.currentSrc)) {
        Application.App.log.Info(
    ...
    if (!this.autoResumeListenersAdded) {
      ...
      this.addManagedListener(
        ...
        () => {
          ...
          if (
            this.pauseTimestamps.length >= ZSGL_CONSTANTS.PAUSE_STORM_MAX_COUNT
          ) {
            this.abortAutoResume();
            Application.App.log.Error(
  ...
  /** 设置视频结束处理:结束后按平台确认状态决定完成或重播 */
  private setupVideoEndHandler(): void {
    this.addManagedListener(this.video, "ended", () => {
      Application.App.log.Info("[视频事件] 视频播放结束");
      if (this.platformConfirmed) {
        this.finishTask("视频播放结束且平台已确认");
        return;
      }
      // 播放完了但未拦截到 setContentFinished(平台未确认完成) → 重播
      Application.App.log.Info(
        "[完成检测] 视频已播放完但未拦截到 setContentFinished,自动重播",
      );
      this.video.currentTime = 0;
      if (Application.App.config.auto) {
        this.clickPlayButton();
  ...
  /** 拦截平台完成标志接口:收到 setContentFinished 响应即视为任务完成 */
  private setupFinishHook(): void {
    hookHttpRequest(
      ZSGL_CONSTANTS.HTTP_ENDPOINTS.SET_CONTENT_FINISHED,
      (response, context, url) => {
        if (this.platformConfirmed || this.finishTriggered) {
          return;
        }
        this.platformConfirmed = true;
        Application.App.log.Info(
          "[完成检测] 已拦截到 setContentFinished,平台确认任务完成",
        );
  ...
  /** 统一完成链路:清理挂机资源,写完成标志并推进下一任务 */
  private finishTask(reason: string): void {
    if (this.finishTriggered) {
      return;
    }
    this.finishTriggered = true;
    Application.App.log.Info(`[任务完成] ${reason},进入下一任务`);
    this.timerManager.clearInterval("videoAutoResume");
    this.autoResumeActive = false;
    this.stopKeepAlive();
